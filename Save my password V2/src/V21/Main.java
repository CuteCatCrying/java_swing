...
class Windows extends MenuBar{
	...
	JButton btnEntry,btnCheck,btnDelete;
	JTable tbl;
	JScrollPane sp;
	...
	Windows(){
		...
		mainFrame.setTitle("Save my password V2.1");
		...
		btnEntry.setBounds(293, 33, 100, 25);
		btnCheck.setBounds(293, 65, 100, 25);
		btnDelete.setBounds(293, 97, 100, 25);
			...
			public void actionPerformed(ActionEvent e){
				...
				if(akun.equals("") || username.equals("") || password.equals("")){
					...
					try{
						...
						Statement st = conn.createStatement();
						String sql = String.format("delete from accounts where akun = '%s' and username = '%s' and password = '%s';",akun,username,password);

						st.executeUpdate(sql);
						peringatan.showMessageDialog(mainFrame,"Successfully Deleted","Alert",JOptionPane.WARNING_MESSAGE);
					}catch(Exception exc){
						peringatan.showMessageDialog(mainFrame,"Invalid data","Warning",JOptionPane.WARNING_MESSAGE);
					}
				}
			}
		});
